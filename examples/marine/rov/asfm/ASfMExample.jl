...
init = MvNormal([0.0;0.0;0.0;-0.00772052; 0.0; -0.0992321],initCov)
v1 = addVariable!(fg, :x1,  zeros(6,N) ⊕ Pose3Pose3(init ),  N=N)
initPosePrior = PriorPose3(init)
...
# vl1 = addVariable!(fg, :l1,  0.1*randn(3,N),  N=N)
...
# addVariable!(fg, :l2,  0.1*randn(3,N),  N=N)
...
# addVariable!(fg, :l3,  0.1*randn(3,N),  N=N)
...
# addVariable!(fg, :l4,  0.1*randn(3,N),  N=N)
...
# addVariable!(fg, :l5,  0.1*randn(3,N),  N=N)
...
# addVariable!(fg, :l6,  0.1*randn(3,N),  N=N)
...
# addVariable!(fg, :l7,  0.1*randn(3,N),  N=N)
...
solveTree!(fg)
visualize(fg, vc, N=N, drawtype=:fit, gt=gt)
