...
v1 = addVariable!(fg,:x1,ContinuousScalar,N=N, tags=[:POSE;])
...
v2 = addVariable!(fg, :x2, ContinuousScalar, N=N,tags=[:POSE;])

# now add the odometry factor between them
f1 = addFactor!(fg,[v1;v2],Odo([50.0]',[2.0]',[1.0]))


v3=addVariable!(fg,:x3,4.0*randn(1,N)+getVal(v2)+50.0, N=N,tags=[:POSE;])
addFactor!(fg,[v2;v3],Odo([50.0]',[4.0]',[1.0]))
f2 = addFactor!(fg,[v3], Obsv2(doors, cov', [1.0]))

v4=addVariable!(fg,:x4,2.0*randn(1,N)+getVal(v3)+50.0, N=N,tags=[:POSE;])
addFactor!(fg,[v3;v4],Odo([50.0]',[2.0]',[1.0]))

# if true
l1=addVariable!(fg, :l1, 0.5*randn(1,N)+getVal(v3)+64.0, N=N,tags=[:LANDMARK;])
addFactor!(fg, [v3,l1], Ranged([64.0],[0.5],[1.0]))
addFactor!(fg, [v4,l1], Ranged([16.0],[0.5],[1.0]))
# end


v5=addVariable!(fg,:x5,2.0*randn(1,N)+getVal(v4)+50.0, N=N,tags=[:POSE;])
addFactor!(fg,[v4;v5],Odo([50.0]',[2.0]',[1.0]))


v6=addVariable!(fg,:x6,1.25*randn(1,N)+getVal(v5)+40.0, N=N,tags=[:POSE;])
addFactor!(fg,[v5;v6],Odo([40.0]',[1.25]',[1.0]))


v7=addVariable!(fg,:x7,2.0*randn(1,N)+getVal(v6) +60.0, N=N,tags=[:POSE;])
