...
for i in range(NUM_ODOM_NODES):
    ...
    if old_odom_num == 0:
        session.run("MERGE (o1:POSE:SESSROX:NEWDATA {slam_info: {var_info1} }) " # finds, creates if not exist
                    "MERGE (f:FACTOR:SESSROX:NEWDATA { slam_info: {fac_info} }) " # prior factor
                    "MERGE (o1)-[:REL]-(f)",
    ...
    session.run("MERGE (o1:POSE:SESSROX:NEWDATA {slam_info: {var_info1} }) " # finds, creates if not exist
                "MERGE (o2:POSE:SESSROX:NEWDATA { slam_info: {var_info2} })"
                "MERGE (f:FACTOR:SESSROX:NEWDATA { slam_info: {fac_info} }) " # odom-odom factor
    ...
    if add_landmark:
        ...
        session.run("MERGE (l1:LANDMARK:SESSROX:NEWDATA {slam_info: {land_info}})"
                    "MERGE (o1:POSE:SESSROX:NEWDATA {slam_info: {var_info}})"
                    "MERGE (f:FACTOR:SESSROX:NEWDATA {slam_info: {fac_info}})" # odom-landmark factor
                    "MERGE (o1)-[:REL]->(f) "
                    "MERGE (f)-[:REL]->(l1) ",
